...
def AnalizRyada(file_content, file_type):
    ...
    for l in range(1, p+1):
        ...
        new_row = pd.DataFrame({'Дата': [GetDate()], 'Цена нефти, $': [round(listL[len(listL)-1]+k*listT[len(listT)-1], 2)]})
...
async def aniliz(file: UploadFile = File(...)):
    file_content = await file.read()  # Чтение содержимого файла в память
    file_type = file.content_type
    ds, end_date = AnalizRyada(file_content, file_type)
